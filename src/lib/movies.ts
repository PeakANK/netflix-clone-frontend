...
export const searchMovies = (q:string, page=1) => apiGet<PagedResponse<Movie>>("/movies/search", { q, page });
export const getMovieDetails = (id:number, append?:string[]) => apiGet<any>(`/movies/${id}`, { append: append?.join(",") });
export const getMovieCredits = (id:number) => apiGet<any>(`/movies/${id}/credits`);
export const getMovieVideos  = (id:number) => apiGet<any>(`/movies/${id}/videos`);
