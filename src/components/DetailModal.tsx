...
import type { MediaDetails, Video, Cast } from '@/types/media';
...
export default function DetailModal({ open, onClose, mediaType, id }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await apiGet<MediaDetails>(endpoint, { language: 'en-US' });
  ...
  const seasons = mediaType === 'tv' ? (data?.number_of_seasons ? `${data.number_of_seasons} seasons` : null) : null;
          ...
          <>
            <div className="flex flex-wrap items-center gap-3">
              <h2 className="text-2xl md:text-3xl font-extrabold">
                {title} {year && <span className="text-white/50 text-xl font-normal">({year})</span>}
              </h2>
              {data.id && (
            ...
            <div className="mt-2 text-white/80 text-sm flex flex-wrap gap-x-3 gap-y-1">
              ...
              {typeof data.vote_average === 'number' && <span>⭐ {Math.round(data.vote_average * 10) / 10}</span>}
              {data.genres?.length ? <span>{data.genres.map(g => g.name).join(', ')}</span> : null}
            </div>

            {data.overview && <p className="mt-4 text-sm leading-6 text-white/85">{data.overview}</p>}
                ...
                <div className="flex gap-4 overflow-x-auto no-scrollbar pr-2">
                  {cast.map(c => (
                    <div key={c.id} className="shrink-0 w-[120px]">
                      ...
                      <div className="mt-1 text-xs font-semibold line-clamp-2">{c.name}</div>
                      {c.character && <div className="text-[11px] text-white/60 line-clamp-1">{c.character}</div>}
                    </div>
                  ))}
                </div>
              </div>
