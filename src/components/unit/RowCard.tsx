...
function titleOf(i: MediaItem) { return "title" in i ? i.title : i.name; }
...
export default function RowCard({
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLAnchorElement>) {
    if (!onOpen) return;
    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onOpen(item.id, item); }
  }
  ...
  return (
    ...
    >
      <div
        className="relative shrink-0 aspect-[2/3] overflow-hidden rounded-md bg-neutral-800/50 ring-1 ring-white/5
      ...
      >
        {url ? (
          <Image
            ...
            sizes="(min-width:1536px) 10vw,(min-width:1280px) 12vw,(min-width:1024px) 16vw,(min-width:768px) 22vw,(min-width:640px) 30vw,44vw"
          />
        ) : (
